...
# polish_comment（【完成版】）
- 完成版で変えたが**減点対象ではない**表現のみ（自然な明確化・文体調整・冗長整理など）。
- 各行: `● 原文 → 完成版：理由`（**減点表記禁止**。➖0点表記も禁止）
...
下書きの完成版英文を、内容の指摘（①良い点・②改善点・③減点箇所）と文法修正一覧に**すべて忠実に反映**した完成版に書き直してください。

# 厳守
- 生徒が本当に言いたかった意図を失わない。設問条件（語数・必須要素）を満たす。
- **②改善点**・**③減点箇所**で述べた内容面の改善は、完成版英文に**必ず実装**する。
...
def content_comment_has_required_sections(content_comment: str) -> bool:
    cc = content_comment or ""
    return all(m in cc for m in ("①良い点", "②改善点", "③減点箇所"))
