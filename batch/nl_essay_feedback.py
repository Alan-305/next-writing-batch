...
5. 内容で減点した観点には、該当箇所の文末に**必ず**実際の減点幅を `（➖2点）`〜`（➖5点）` で付ける（同一観点で重複減点しない。減点なしなら付けない）。1行目の内容得点および JSON の `content_deduction` と整合させる。
内容面で改善提案をした場合、その提案は完成版英文に必ず反映すること。

- **配置の厳守**：意図の言語化・①②③・【ヒント】は**すべて content_comment のみ**。本文の●行（JSON より前）は**文法専用**。
- content_comment では**よしあし**をはっきり述べる（冠詞・綴り・時制・一致・語順・前置詞・品詞・接続の誤りなど**形・用法の指摘は絶対に書かない**）。内容で減点した観点の文末には実際の減点幅を `（➖2点）`〜`（➖5点）` で明記する。
- **【ヒント】**を1行の見出しとし、その次行に Step3 の具体を書くこと。
- **良い場合**：各観点について具体的に認め、**「とても素晴らしいです」レベルの力強い称賛**を交えて前向きに伝える（空疎な一言だけにしない）。
- **改善が必要な場合**：どこがどう問題かを明示し、**どう書き換えればよいか**をコンパクトに提案する。提案は**完成版の英文に必ず反映**する（内容の指摘だけで終わらせない）。**【ヒント】で「こうすればよくなる」と述べる内容も、今回の完成版英文に実装すること**（次回向けの一般論だけにしない）。
...
- grammar_comment は文法・語法・表現の指摘をそれぞれ箇条書きにし、各項目の行頭は必ず「● 」（中黒＋半角スペース）から始める。**各行の末尾に（➖X点）を必ず付ける**（本文の●行と同一・省略禁止。X は整数。同一誤りの再出は（➖0点）可）。**減点対象でない書き換えは（➖0点）とし、理由を必ず書く。減点対象ではないのに（➖1点）（➖2点）を付けない。**
...
- JSON を閉じる前に自己確認：（1）`content_comment` に「●」が含まれていない（2）③が句点で終わっている（3）**【ヒント】**見出しとその次の段落がある（4）`content_comment` の文字列が途中で切れていない（5）【ヒント】で述べた改善が**すでに完成版英文に実装されている**。
- **禁止**：content_comment を「総評や得点を参照してください」「チェックリストで確認」などの**誘導だけ**にすること。①②③はそれぞれ**この答案について具体的**によしあしを**2文以上**で書くこと（観点名だけ並べない）。
- **【ヒント】**は**この答案の英文・設問に即した**、**今回の完成版英文に盛り込むべき**具体的な書き方・表現の改善アドバイスにすること（汎用の最終確認・チェックリスト化だけの文は禁止）。完成版を書いたあとで、反映されていないアドバイスだけを【ヒント】に書くことは禁止。

# 指摘と完成版の対応（厳守・最重要）
ベテラン講師として「**指摘してから修正する**」を徹底すること。完成版英文で原文から変えた箇所は**すべて**、文法の●行または `content_comment`（①②③・【ヒント】）のいずれかに**必ず**指摘すること。
完成版にだけ存在し、指摘に無い修正（無言修正・コメントなしの書き換え）は**禁止**（信頼を損なう）。
形・用法・冠詞・語法・表現・語彙の言い換えは文法の●行へ。論旨・具体性・論理・結論の補強は `content_comment` へ。
完成版を書き終えたら、原文との差分を一つずつ確認し、指摘漏れがあれば追記してから出力を確定すること。
"""
...
下書きの完成版英文を、内容の指摘（①②③・【ヒント】）と文法修正一覧に**すべて忠実に反映**した完成版に書き直してください。

# 厳守
- 生徒が本当に言いたかった意図を失わない。設問条件（語数・必須要素）を満たす。
- 内容の指摘で「〜するとよい」「〜を足すとよい」「〜が不足」「〜を明確に」など改善を述べている点は、完成版英文に**必ず実装**する。
- **【ヒント】**で述べた「こうすればよくなる」書き方・表現・論理の補強は、**今回の完成版英文に必ず盛り込む**（一般論だけにしない）。
...
- 既存の文法●行・content_comment に**すでに**書いてある修正は**重複して書かない**。
- **than と比較級**: 原級＋than（例: high value ... than earning）を直して rather than を採用した場合、**比較級案（higher ... than）と rather than 採用の理由**を必ず追記する（漏れがあれば additional_grammar_bullets へ）。
- **時の副詞と時制**: in recent years 等を入れた・直したら、**動詞を現在完了／現在完了進行形に合わせた理由**も必ず追記する（時制変更が完成版にあるのに言及が無ければ追記）。
...
- 軽微な文法誤りは（➖1点）、重要な誤りは（➖2点）。同一誤りの再出は（➖0点）可。
- **減点対象でない書き換え**（自然な言い換え・明瞭化のみ）は**（➖0点）**とし、**書き換えた理由を必ず明記**する。くれぐれも減点しない。
...
{{"additional_grammar_bullets":"","content_comment_additions":""}}
...
def _insert_content_additions_before_hint(content_comment: str, additions: str) -> str:
    ...
    if not cc:
        return add
    hint_idx = cc.find("【ヒント】")
    if hint_idx >= 0:
        before = cc[:hint_idx].rstrip()
        after = cc[hint_idx:]
        return f"{before}\n\n{add}\n\n{after}"
    return f"{cc}\n\n{add}"
...
def parse_feedback_coverage_audit_response(raw: str) -> Dict[str, str]:
    ...
    s = (raw or "").strip()
    if not s:
        return {"additional_grammar_bullets": "", "content_comment_additions": ""}
...
def merge_feedback_coverage_audit(
    *,
    ...
    戻り値: body_explanation, content_comment, grammar_comment, content_deduction, grammar_deduction
    """
    ...
    combined_grammar = _combined_grammar_bullets_text(body, gc)
    ...
    return body, cc, gc, cd_new, gd_new
