...
# content_comment（【内容】）— コンパクト3区分のみ（厳守）
次の3見出し**のみ**（この順）。各見出しの下は `・` 箇条書き（各1行・簡潔）。**中黒「●」禁止**。
1. `①良い点` … 答案の良い点（最大4項目）
2. `②改善点` … 内容面で完成版に反映した改善（最大4項目）
3. `③減点箇所` … **内容減点した箇所のみ**（各行末（➖N点）。無ければ `・（該当なし）`）
**禁止**: 意図の言語化の長段落、【ヒント】、旧「①設問の条件…」形式の長文、文法●行との重複。

# grammar_comment（【文法・語法・表現】）
- **減点あり（➖1点以上）の●行のみ**。`（➖0点）` や減点表記のない行は**絶対に入れない**（polish_comment へ）。
- 各行: `● 誤り → 正：解説（➖X点）`（X≧1）

# polish_comment（【完成版の書き換え】）
- 完成版で変えたが**減点対象ではない**表現のみ。
- 各行: `● 原文 → 完成版：理由`（**減点表記禁止**）
...
- **減点対象でない書き換え** → `additional_polish_bullets`（●行、減点表記禁止）
...
def content_comment_has_required_sections(content_comment: str) -> bool:
    ...
    if all(m in cc for m in ("①良い点", "②改善点", "③減点箇所")):
        return True
    return all(m in cc for m in ("①", "②", "③")) and ("【ヒント】" in cc or "設問" in cc)
...
_POLISH_SECTION_HEAD_PY = "【完成版の書き換え】"
_HINT_HEAD_PY = "【ヒント】"
...
def normalize_student_explanation_text(text: str) -> str:
    ...
    def is_polish_head(s: str) -> bool:
        return trimmed(s) == _POLISH_SECTION_HEAD_PY
