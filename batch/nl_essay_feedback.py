...
- **箇条書きは生徒の原文の登場順（上から下・先頭の文から順）に並べる**（英文の流れに沿い、読み手が該当箇所と照合しやすい順序を厳守）。
...
- 各行: 書き換えは `・ 原文 → 完成版：理由`、**追加は `・（追記）追加した英語：理由`**（**減点表記禁止**）。理由は必須。矢印は `→`（`->` 禁止）。
- **【完成版で新たに加えた英文・表現は必ず理由付きで列挙・最重要】** 原文になかった文・語句を完成版で加えたら、なぜ加えたか（例: 主張を具体例で補強するため／結論を明確にするため／論理をつなぐため）を読み手が理解できる具体的な日本語で書く。
- 「完成版で整えました」等の**中身のない理由は禁止**。何をどう良くしたか具体的に書く。
...
# 重複指摘・二重減点の禁止（grammar_comment 厳守・最重要）
- **同じ修正方針を2行以上書かない**（例: `high school` → `high schools`、`Not only you can ... but also ...` の倒置が答案に複数あっても●行1本・減点1回のみ）
- **同じ内容の指摘は、原文の引用範囲・言い回しが違っても『同一』とみなし、●行1本・減点1回のみ**。同じ直し（→ の右側＝修正後の英語が実質同じ）を少し表現を変えて2行に分け、2回減点することを固く禁じる。
- 出力前に、修正後の英語（→ の右側）が実質同じ●行が2本以上ないか必ず確認し、あれば1本に統合して減点も1回にする。
- 説明文が同じ指摘を繰り返すことも禁止
...
- JSON 文字列内改行は `\\n` でエスケープ。
- 自己確認: grammar に➖0点無し / polish に減点表記無し / 完成版の変更が3ブロックでカバーされている / grammar の●行が原文の登場順 / A4 2ページ以内
...
- **【二重減点の禁止・最重要】** 既存の●行と**修正後の英語（→ の右側）が実質同じ**なら、引用範囲や言い回しが違っても「既出」とみなし `additional_grammar_bullets` に追記しない（同じ直しで2回目の減点を発生させない）。追記は、既存指摘に**無い新しい種類の誤り**のみ。
...
- **Not only you can ... but also ...** の倒置も既存に1行でもあれば追記しない（`to feed him` 等の後続語句の差は無視して1回）。
...
- **減点対象でない書き換え・追記** → `additional_polish_bullets`（書き換えは `・ 原文 → 完成版：理由`、**追加した英文・表現は `・（追記）追加した英語：なぜ加えたか`** の形式。減点表記禁止。理由必須）
- **完成版で原文になかった英文・表現を新たに加えた箇所は、必ず `additional_polish_bullets` に理由付きで挙げる**（例: 主張を具体例で補強するため／結論を明確にするため／論理をつなぐため）。読み手が「なぜこの英文があるか」を理解できる具体的な日本語にし、「整えました」等の空虚な理由は禁止。
...
def _grammar_fix_phrase(line: str) -> str:
    """●行の「→ 正しい形」側（修正後フレーズ）を正規化して返す。

    同じ修正内容を左側（誤り側）の引用範囲だけ変えて2行書いた場合、
    左側キーでは重複と判定できず二重減点になる。修正後フレーズを
    追加のキーにすることで、同一修正の二重計上を防ぐ。
    """
    norm = _normalize_grammar_arrow_text((line or "").strip())
    m = re.search(r"→\s*`?([^`：:]+)", norm)
    if not m:
        return ""
    fix = re.split(r"[：:（(]", m.group(1))[0]
    return re.sub(r"\s+", " ", fix.lower().strip())


def _grammar_correction_signature(line: str) -> str:
    """同一の訂正ルール（例: `depend of` → `depend on`）を対象語が違っても
    1つにまとめるための署名キー。

    誤り側と修正側の差分が1箇所・局所的で、その直前に共通のアンカー語
    （動詞など）がある場合に `rule:アンカー:誤り語->修正語` を返す。説明文の
    書きぶりに依存しないため、同じ直しを別々の語に当てて2回減点する二重減点を
    確実に防げる。過剰統合を避けるため、局所的な変更のみを対象にする。
    """
    pair = _arrow_pair_from_grammar_line(line)
    if not pair:
        return ""
    err_w = re.findall(r"[A-Za-z]+(?:'[A-Za-z]+)?", pair[0].lower())
    fix_w = re.findall(r"[A-Za-z]+(?:'[A-Za-z]+)?", pair[1].lower())
    if not err_w or not fix_w:
        return ""
    ops = [
        op
        for op in difflib.SequenceMatcher(None, err_w, fix_w).get_opcodes()
        if op[0] != "equal"
    ]
    if len(ops) != 1:
        return ""
    ...
    if (i2 - i1) > 2 or (j2 - j1) > 2:
        return ""
    err_seg = " ".join(err_w[i1:i2])
    fix_seg = " ".join(fix_w[j1:j2])
    anchor = err_w[i1 - 1] if i1 > 0 else (fix_w[j1 - 1] if j1 > 0 else "")
    if len(anchor) < 3:
        return ""
    return f"rule:{anchor}:{err_seg}->{fix_seg}"
...
def _grammar_correction_stem_key(line: str) -> str:
    """同一文法ミス（語句の長さだけ違う重複）を1本にまとめるキー。"""
    pair = _arrow_pair_from_grammar_line(line)
    if not pair:
        return _grammar_bullet_key(line)
    err_l, fix_l = pair[0].lower(), pair[1].lower()
    if re.search(r"\bnot only\b", err_l) and re.search(r"\bnot only can\b", fix_l):
        return "stem:not_only_inversion"
    if "high school" in err_l and "high schools" in fix_l:
        return "stem:high_school_plural"
    if re.search(r"especially\s+people", err_l) or (
        "especially" in err_l and re.search(r",\s*they\b", err_l)
    ):
        return "stem:especially_dangling"
    if "tired to work" in err_l or ("tired to" in err_l and "tired from" in fix_l):
        return "stem:tired_from_work"
    exp = _grammar_explanation_signature(line)
    if exp and len(exp) >= 12:
        err_head = " ".join(err_l.split()[:5])
        return f"stem:exp:{exp[:32]}:{err_head[:24]}"
    return _grammar_error_phrase(line)


def _grammar_lines_are_near_duplicates(a: str, b: str) -> bool:
    """誤り側が包含関係＋説明が同系統なら重複とみなす。"""
    sk_a, sk_b = _grammar_correction_stem_key(a), _grammar_correction_stem_key(b)
    if sk_a == sk_b and sk_a.startswith("stem:"):
        return True
    pa, pb = _arrow_pair_from_grammar_line(a), _arrow_pair_from_grammar_line(b)
    if not pa or not pb:
        return _grammar_bullet_key(a) == _grammar_bullet_key(b)
    ea, eb = pa[0].lower(), pb[0].lower()
    fa, fb = pa[1].lower(), pb[1].lower()
    if ea == eb:
        return True
    if ea in eb or eb in ea:
        exp_a, exp_b = _grammar_explanation_signature(a), _grammar_explanation_signature(b)
        if exp_a and exp_b and (exp_a == exp_b or exp_a in exp_b or exp_b in exp_a):
            return True
        if fa[:48] == fb[:48]:
            return True
    return False
...
def _correction_rule_group_id(line: str) -> Optional[str]:
    ...
    if re.search(r"\bnot only\b", err_l) and re.search(r"\bnot only can\b", fix_l):
        return "corr:not_only_inversion"
    if "複数" in (line or "") and "high school" in err_l:
        return "corr:high_school_plural"
    # 説明文の書きぶりに依存しない訂正署名で、同じ直しを対象語が違っても1本に統合する
    # （説明文ベースのキーは、AIの言い回しの揺れで二重減点を招くため使わない）。
    sig = _grammar_correction_signature(line)
    if sig:
        return sig
    return None
...
_NOT_ONLY_INVERSION_CANONICAL = (
    "● `Not only you can ... but also ...` → "
    "`Not only can you ... but you can also ...`："
    "Not only で文頭を始めるときは倒置（can you）が必要。"
    "but also の後にも主語＋助動詞（you can 等）を補う。"
    "答案内に同じ構文ミスが複数あっても減点はこの指摘1回のみ。（➖2点）"
)

_CORRECTION_RULE_CANONICAL = {
    "corr:high_school_plural": _HIGH_SCHOOL_PLURAL_CANONICAL,
    "corr:not_only_inversion": _NOT_ONLY_INVERSION_CANONICAL,
}
...
def _prune_near_duplicate_grammar_lines(lines: List[str]) -> List[str]:
    """語句長だけ違う同一指摘・同一説明の重複行を除去（最も情報量の多い1行を残す）。"""
    if len(lines) < 2:
        return lines
    kept: List[str] = []
    for ln in lines:
        ...
        for i, prev in enumerate(kept):
            if _grammar_lines_are_near_duplicates(prev, ln):
                if len(ln or "") > len(prev or ""):
                    kept[i] = ln
                merged = True
                break
        if not merged:
    ...
    for tag, i1, i2, j1, j2 in sm.get_opcodes():
        ...
        if tag == "replace":
            ...
            _add(
                "",
                f_chunk,
                "主張の根拠や具体例を補い、内容をより充実させるために完成版で加えた文です",
            )
...
def collapse_duplicate_grammar_bullets(text: str) -> str:
    ...
    for ln in lines:
        ...
        if gid:
            ...
            stem = _grammar_correction_stem_key(ln)
            err = _grammar_error_phrase(ln)
            keys: List[str] = []
            if stem.startswith("stem:"):
                keys.append(stem)
            keys.append(err)
            fix = _grammar_fix_phrase(ln)
            if len(fix) >= 10:
                keys.append(f"fix:{fix}")
            sig = _grammar_correction_signature(ln)
            if sig:
                keys.append(sig)
            if any(k in seen_u for k in keys):
                continue
            for k in keys:
                seen_u.add(k)
            out.append(ln)
    out = _prune_near_duplicate_grammar_lines(out)
    return "\n".join(out).strip()


def _grammar_bullet_position_in_essay(line: str, original_essay: str) -> int:
    """原文中で該当誤りが最初に現れる位置（見つからなければ末尾扱い）。"""
    essay = (original_essay or "").strip()
    if not essay:
        return 10**9
    ...
    err = _grammar_error_phrase(line)
    if err and err not in candidates:
        candidates.append(err)

    best = 10**9
    for cand in candidates:
        c = re.sub(r"\s+", " ", (cand or "").strip().lower())
        if len(c) < 3:
            continue
        pos = essay_lower.find(c)
        if pos >= 0:
            best = min(best, pos)
            continue
        c2 = c.strip("`'\"").strip()
        if c2 != c:
            pos = essay_lower.find(c2)
            if pos >= 0:
                best = min(best, pos)
                continue
        words = re.findall(r"[a-z]+(?:'[a-z]+)?", c2)
        if len(words) >= 2:
            for n in range(min(len(words), 6), 1, -1):
                sub = " ".join(words[:n])
                pos = essay_lower.find(sub)
                if pos >= 0:
                    best = min(best, pos)
                    break
    return best
...
def sort_grammar_bullets_by_essay_order(text: str, original_essay: str) -> str:
    """grammar ●行を生徒の原文の出現順（上から下）に並べ替える。"""
    lines = _extract_grammar_bullet_lines(text)
    if len(lines) < 2 or not (original_essay or "").strip():
        return (text or "").strip()
    indexed = [
        (i, _grammar_bullet_position_in_essay(ln, original_essay), ln)
        for i, ln in enumerate(lines)
    ]
    indexed.sort(key=lambda t: (t[1], t[0]))
    return "\n".join(ln for _, _, ln in indexed).strip()
...
def finalize_grammar_and_polish_blocks(
    *,
    ...
    ded = sort_grammar_bullets_by_essay_order(ded, original_essay)
...
def _grammar_dedup_keys(line: str) -> List[str]:
    stem = _grammar_correction_stem_key(line)
    err = _grammar_error_phrase(line)
    keys: List[str] = []
    if stem.startswith("stem:"):
        keys.append(stem)
    keys.append(err)
    keys.append(_grammar_bullet_key(line))
    fix = _grammar_fix_phrase(line)
    if len(fix) >= 10:
        keys.append(f"fix:{fix}")
    sig = _grammar_correction_signature(line)
    if sig:
...
def _append_grammar_bullets_deduped(existing: str, additional: str) -> str:
    ...
    if not add_lines:
        return (existing or "").strip()
    seen: set[str] = set()
    for ln in _extract_grammar_bullet_lines(existing):
        for k in _grammar_dedup_keys(ln):
            seen.add(k)
    out_lines = [ln for ln in (existing or "").splitlines() if ln.strip()]
    for ln in add_lines:
        keys = _grammar_dedup_keys(ln)
    ...
    return collapse_duplicate_grammar_bullets(merged)
