...
- **Day4 音声**: Google Cloud Text-to-Speech（WaveNet、既定 `en-US-Wavenet-F`）。API 有効化と ADC / `GOOGLE_APPLICATION_CREDENTIALS` が必要です。
...
- GCS 利用時: 音声を GCS に上げ、**`NWB_PUBLIC_APP_URL` / `AUDIO_BASE_URL` があるときは** `/api/day4-audio/...` を QR に埋め込む（**既定 30 日間** 再生可・`DAY4_AUDIO_RETENTION_DAYS`）。無いときのみ署名付き URL（V4 最大 **7** 日）。確認: `./.venv/bin/python3 batch/verify_gcs_setup.py`（`batch/OPERATIONS_5MIN.md` **0.5**）
