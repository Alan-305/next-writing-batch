...
export default function OpsGeminiKeyPage() {
  ...
  const sourceLabel =
    status?.source === "env"
      ? "環境変数（.env.local など）"
      : status?.source === "file"
        ? "保存ファイル（data/gemini_api_key.txt）"
        : "未設定";
    ...
    <main>
      ...
      <div className="card">
        <p className="muted" style={{ marginTop: 0 }}>
          提出一覧の「添削」や課題文の Gemini 取り込みで使います。<strong>環境変数</strong>{" "}
          <code>GEMINI_API_KEY</code>（または <code>GOOGLE_API_KEY</code>）が<strong>優先</strong>され、無いときだけ{" "}
          <code>data/gemini_api_key.txt</code> の1行目を読みます。キーは<strong>平文</strong>なので Git にコミットしないでください（
        ...
        <form style={{ padding: 14, marginBottom: 12, border: "1px solid #e2e8f0", borderRadius: 8 }} onSubmit={onSave}>
          <label className="field">
            ...
            <input
              ...
              placeholder="AIza… または Google のキー"
              autoComplete="off"
              disabled={busy}
            />
          </label>
